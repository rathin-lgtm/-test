...
Join {{ source('raw_from_harborview_edw', 'dim_fund') }} d
on p.fund_id = d.fund_id
join {{ source('raw_from_harborview_edw', 'fact_fund_sub_perspective_funds') }} f
on d.fund_id = f.fund_id
join {{ source('raw_from_harborview_edw', 'dim_fund_sub_perspective') }} sp
