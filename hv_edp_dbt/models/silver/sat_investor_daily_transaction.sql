WITH distribution_transactions AS(
    ...
    FROM {{ source('raw_from_harborview_edw', 'fact_investor_transactions') }} AS inv_tr
    JOIN    {{ ref('link_fund_investor_transaction') }} AS link 
        ...
        LEFT JOIN {{ source('raw_from_harborview_edw', 'calendar_quarter') }} AS cal_q
        ON inv_tr.date_id = cal_q.quarter_id
        JOIN {{ source('raw_from_harborview_edw', 'dim_fund') }} dim_fund
...
investor_withholding_transaction AS(
    SELECT hk_link,
    inv_tr.date_id,
        cal_q.quarter_id,
        cal_q.quarter_desc,
...
final_metrics AS(
    ...
    SELECT hk_link,
        ...
        transaction_type,
        load_dt
    FROM investor_withholding_transaction
)
