with attributes as (
    ...
    FROM
        {{ source('raw_from_harborview_edw', 'dim_investor') }} investor
    LEFT JOIN {{ source('raw_from_harborview_edw', 'dim_investor') }} grp
    ...
    LEFT JOIN {{ source('raw_from_harborview_edw', 'global_edw_key_to_iqid') }} fid
