...
with monthly_metrics as (
    ...
    FROM (
        SELECT * FROM {{ ref('fact_investment_transactions_fund_hierarchy_monthly') }} WHERE date_flag in ('B', 'E') and portfolio_id != -1
    ) transactions_monthly
    JOIN {{ ref('currency') }} currency
        ON transactions_monthly.currency_id = currency.currency_id
    JOIN {{ ref('dim_portfolios') }} dim_portfolios
...
daily_metrics as (
    ...
    FROM (SELECT * FROM {{ ref('fact_investment_transactions_fund_hierarchy') }} 
...
cashflows as (
    SELECT * FROM (
        ...
        FROM {{ ref('fact_irr_investment_fund_hierarchy') }} fii
        JOIN ({{ irr_calendar() }}) cal ON
            (fii.date_id = cal.rollup_date_id)
        JOIN {{ ref('dim_fund_hierarchy') }} dfh ON
