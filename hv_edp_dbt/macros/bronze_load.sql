...
{% macro load_full_refresh_table(source_name, table_name, exclude_cols=['value', 'time_period_key', 'session_log_key', 'year', 'month', 'day'], source_missing_cols=[]) %}
    {% set final_excludes = [] %}
    {% for col in exclude_cols %}
        {% if col not in source_missing_cols %}
            {% do final_excludes.append(col) %}
        {% endif %}
    {% endfor %}

    SELECT * EXCLUDE ({{ final_excludes | join(', ') }}), 
